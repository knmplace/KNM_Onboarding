#!/bin/bash
# =============================================================================
# Homestead — Self-Hostable Onboarding Management Platform
...
# ─── Phase 0: Pre-flight ─────────────────────────────────────────────────────
header "Homestead Installer — Pre-flight"
...
echo "  • Build and start Homestead on port 6001"
...
if prompt_yn "Do you have an existing PostgreSQL instance?"; then
  ...
  prompt "  Database name [homestead]:" DB_NAME_INPUT
  DB_NAME="${DB_NAME_INPUT:-homestead}"
  prompt "  Database user [homestead_user]:" DB_USER_INPUT
  DB_USER="${DB_USER_INPUT:-homestead_user}"
...
# Homestead Environment — generated by deploy.sh on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
...
# ─── Phase 6: Build ──────────────────────────────────────────────────────────
header "Phase 6: Building Homestead"
unset NODE_ENV
npm run build
...
success "Homestead started via PM2 as '${PM2_APP_NAME}'."
...
if prompt_yn "Set up automatic deployment from a git repository?"; then
  ...
  if [[ "$GIT_PROVIDER" == "1" || "$GIT_PROVIDER" == "2" ]]; then
    ...
    if ! id -u homestead &>/dev/null; then
      ...
      success "Created system user 'homestead' for webhook service."
    fi
    # Grant homestead user read access to .env.local (owned by root, 600)
    # by creating a group-readable copy — simpler: just set group homestead + 640
    chown root:homestead "${INSTALL_DIR}/.env.local"
    chmod 640 "${INSTALL_DIR}/.env.local"

    # Write systemd service
    SERVICE_NAME="webhook-homestead"
    cat > "/etc/systemd/system/${SERVICE_NAME}.service" <<SVCEOF
[Unit]
Description=Homestead Git Webhook Server
...
echo -e "${GREEN}${BOLD}"
echo "  ╔══════════════════════════════════════════╗"
echo "  ║     Homestead Successfully Deployed      ║"
echo "  ╚══════════════════════════════════════════╝"
echo -e "${RESET}"
...
if [[ "$SETUP_REQUIRED" == "true" ]]; then
  ...
  echo -e "${RESET}"
  echo "  Some credentials were skipped. Before using Homestead, visit:"

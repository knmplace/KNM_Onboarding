...
async function loadSite(client, { siteId, siteSlug }) {
  if (Number.isInteger(siteId)) {
    const result = await client.query(
      `
        SELECT id, slug, name, n8n_webhook_auth_key, n8n_sync_workflow_id, n8n_reminder_workflow_id
        FROM site
        WHERE id = $1
        LIMIT 1
      `,
...
async function main() {
  ...
  try {
    ...
    const authKey = clean(site.n8n_webhook_auth_key);
    if (!authKey) {
      throw new Error(`Site ${site.slug} is missing n8n_webhook_auth_key.`);
    }

    const verification = await verifyProvisionedSite(
      site,
      authKey,
      INTERNAL_ONBOARDING_API_BASE.replace(/\/$/, "")
    );
      ...
      JSON.stringify(
        {
          ...
          site: {
            ...
            slug: site.slug,
            name: site.name,
            syncWorkflowId: site.n8n_sync_workflow_id,
            reminderWorkflowId: site.n8n_reminder_workflow_id,
          },
          appUrl: INTERNAL_ONBOARDING_API_BASE.replace(/\/$/, ""),
          verification,
        },
        null,
        2

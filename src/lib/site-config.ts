...
export async function buildSiteConfigFromInput(
  ...
  return {
    siteData: {
      ...
      wordpressUsername: input.wordpressUsername?.trim() || null,
      wordpressAppPassword: input.wordpressAppPassword?.trim() || null,
      profilegridApiUrl: `${siteUrl}/wp-json/profilegrid/v1`,
      profilegridUsername: input.wordpressUsername?.trim() || null,
      profilegridAppPassword: input.wordpressAppPassword?.trim() || null,
